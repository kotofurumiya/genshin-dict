import type { DictBase } from '../../dict.d.ts';

export default {
  category: '人物',
  title: 'フォンテーヌ',
  hinshi: '人名',
  items: [
    // 実装済み・実装予定プレイアブル関係
    { hiragana: 'りね', word: 'リネ' },
    { hiragana: 'りねっと', word: 'リネット' },
    { hiragana: 'ふれみね', word: 'フレミネ' },
    { hiragana: 'ふぉかろるす', word: 'フォカロルス' },
    { hiragana: 'しゃるろっと', word: 'シャルロット' },
    { hiragana: 'なゔぃあ', word: 'ナヴィア' },
    { hiragana: 'くろりんで', word: 'クロリンデ' },
    { hiragana: 'しぐうぃん', word: 'シグウィン' },
    { hiragana: 'りおせすり', word: 'リオセスリ' },
    { hiragana: 'ぬゔぃれっと', word: 'ヌヴィレット' },
    { hiragana: 'ふりーな', word: 'フリーナ' },
    { hiragana: 'じぇんとるまん・あっしゃー', word: 'ジェントルマン・アッシャー', note: 'フリーナのサロン・ソリティアのメンバー' },
    { hiragana: 'しゅゔぁるまらんふじん', word: 'シュヴァルマラン婦人', note: 'フリーナのサロン・ソリティアのメンバー' },
    { hiragana: 'くらばれったさん', word: 'クラバレッタさん', note: 'フリーナのサロン・ソリティアのメンバー' },
    { hiragana: 'しゅゔるーず', word: 'シュヴルーズ' },
    { hiragana: 'えみりえ', word: 'エミリエ' },
    { hiragana: 'えすこふぃえ', word: 'エスコフィエ' },

    // ゲーム中や漫画に登場するキャラクター
    { hiragana: 'まるしらっく', word: 'マルシラック' },
    { hiragana: 'しるゔぁ', word: 'シルヴァ' },
    { hiragana: 'きゃたぴらー', word: 'キャタピラー' },

    // 主に歴史・文献に登場
    { hiragana: 'えげりあ', word: 'エゲリア' },
    // { hiragana: 'えりなす', word: 'エリナス' }, // 地名と重複するので除外
    { hiragana: 'ばざる', word: 'バザル' },
    { hiragana: 'えるとん', word: 'エルトン' },
    { hiragana: 'ばざる・えるとん', word: 'バザル・エルトン' },
    { hiragana: 'るね', word: 'ルネ' },
    { hiragana: 'るねどぺとりこーる', word: 'ルネ・ド・ペトリコール' },
    { hiragana: 'るね・ど・ぺとりこーる', word: 'ルネ・ド・ペトリコール' },
    { hiragana: 'なるつぃっせんくろいつ', word: 'ナルツィッセンクロイツ' },
    { hiragana: 'えまにゅえる', word: 'エマニュエル' },
    { hiragana: 'ぎよたん', word: 'ギヨタン' },
    { hiragana: 'えまにゅえるぎよたん', word: 'エマニュエル・ギヨタン' },
    { hiragana: 'あらん', word: 'アラン' },
    { hiragana: 'あらんぎよたん', word: 'アラン・ギヨタン' },
    { hiragana: 'あらん・ぎよたん', word: 'アラン・ギヨタン' },
    { hiragana: 'きかいこう', word: '奇械公', note: 'アラン・ギヨタンの異名' },
    { hiragana: 'まりあん', word: 'マリアン' },
    { hiragana: 'まりあんぎよたん', word: 'マリアン・ギヨタン' },
    { hiragana: 'まりあん・ぎよたん', word: 'マリアン・ギヨタン' },
    { hiragana: 'かーたー', word: 'カーター' },
    { hiragana: 'しぇるびうす', word: 'シェルビウス' },
    { hiragana: 'かーたーしぇるびうす', word: 'カーター・シェルビウス' },
    { hiragana: 'かーたー・しぇるびうす', word: 'カーター・シェルビウス' },
    { hiragana: 'じぇいこぶ', word: 'ジェイコブ' },
    { hiragana: 'いんごるど', word: 'インゴルド' },
    { hiragana: 'じぇいこぶいんごるど', word: 'ジェイコブ・インゴルド' },
    { hiragana: 'じぇいこぶ・いんごるど', word: 'ジェイコブ・インゴルド' },
    { hiragana: 'かーる', word: 'カール' },
    { hiragana: 'かーるいんごるど', word: 'カール・インゴルド' },
    { hiragana: 'かーる・いんごるど', word: 'カール・インゴルド' },
    { hiragana: 'りりす', word: 'リリス' },
    { hiragana: 'せいもあ', word: 'セイモア' },
    { hiragana: 'くりーゔ', word: 'クリーヴ' },
    { hiragana: 'くるせびな', word: 'クルセビナ', note: '先代「召使」' },
    { hiragana: 'かっしおどる', word: 'カッシオドル' },
    { hiragana: 'ぼえてぃうす', word: 'ボエティウス' },
    { hiragana: 'しびら', word: 'シビラ' },
    { hiragana: 'すきゅら', word: 'スキュラ' },
  ]
} as DictBase;